...
Web-Gateway 的核心设计思想是融合这四者的能力。具体做法是：**按照以下步骤为每个站点匹配最合适的检索工具。一旦在某一步骤匹配成功，则直接终止该站点的工具匹配流程**。
...
总而言之，你需要针对任务找到最合适的检索渠道，并逐个进行有用信息检索。当然，若任务中已经明确指定了检索站点，则直接锁定该站点范围即可。锁定范围后，按照以下步骤完成工具与渠道的匹配。

对于网页交互任务，通常会由用户指定好具体网页供你继续匹配以下工具。
...
使用 CDP 命令通过插件开展网页检索和交互。只有在 `web-access` 不可用时，才降级使用 WebSearch、WebFetch、curl、Jina 等原生轻量网页工具。

## Agent 原生检索工具层

在 `web-access` 不可用的情况下，作为最终的兜底方案。
