...
| 浏览器扩展后端 | 类似 OpenCLI Browser Bridge，一次安装扩展后通过本地 daemon 操作 Chrome / Edge，避免每次触发 remote-debugging 授权 |
| CDP Proxy 浏览器操作 | 作为兜底直连用户日常 Chrome，天然携带登录态，支持动态页面、交互操作、视频截帧 |
...
**本地 v2.5.3-webext.1 更新：**
- **浏览器扩展后端** — 新增 `extension/`、`scripts/webext-proxy.mjs` 和 `scripts/check-webext.mjs`。一次性 Load unpacked 并授权扩展后，默认使用 `127.0.0.1:3457`，不再依赖 Chrome remote-debugging 授权弹窗。
- **URL 改 POST body** — `/new` 和 `/navigate` 使用 POST body 传 URL，避免 query/hash 被截断。
...
claude plugin install web-gateway@web-gateway --scope user
```

**方式四：手动**

```bash
git clone https://github.com/eze-is/web-access ~/.claude/skills/web-gateway
```
...
如果扩展未连接，打开 `chrome://extensions` 或 `edge://extensions`，启用 Developer mode，Load unpacked 选择本 skill 的 `extension/` 目录。安装后后续使用无需再点 remote-debugging 运行权限。
...
# 手动运行请替换为实际路径，如 ~/.claude/skills/web-gateway
```

## Proxy API

扩展后端端口是 `3457`，CDP 兜底端口是 `3456`。

扩展后端通过本地 WebSocket 与浏览器扩展通信；CDP 兜底通过 WebSocket 直连 Chrome（兼容 `chrome://inspect` 方式，无需命令行参数启动）。两者都提供 HTTP API：
...
# 页面操作
curl -s -X POST --data-raw 'https://example.com' "$BASE/new"  # 新建 tab
curl -s -X POST --data-raw 'https://example.com' "$BASE/navigate?target=ID"  # 导航
