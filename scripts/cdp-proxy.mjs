...
async function discoverChromePort() {
  ...
  for (const port of commonPorts) {
    const ok = await checkPort(port);
    if (ok) {
      const wsPath = await getDebuggerWsPath(port);
...
function isSafeWritePath(p) {
  ...
  const roots = [os.tmpdir(), process.cwd()].map(normalize);
  ...
  try {
    ...
    // POST /new (body=URL) - 创建新后台 tab
      ...
      if (req.method !== 'POST') {
        ...
        res.end(JSON.stringify({
          error: '/new 使用 POST body 传 URL，避免目标 URL 中的 query/hash 被截断',
          example: "curl -s -X POST --data-raw 'https://example.com' http://127.0.0.1:3456/new",
        }));
        return;
      }
    ...
    // POST /navigate?target=xxx (body=URL) - 导航（自动等待加载）
      ...
      if (req.method !== 'POST') {
        res.statusCode = 400;
        res.end(JSON.stringify({
          error: '/navigate 使用 POST body 传 URL，避免目标 URL 中的 query/hash 被截断',
          example: "curl -s -X POST --data-raw 'https://example.com' 'http://127.0.0.1:3456/navigate?target=ID'",
        }));
        return;
      }
    ...
    // GET /info?target=xxx - 获取页面信息
      ...
      res.end(JSON.stringify({
        ...
        endpoints: {
          ...
          '/setFiles?target=': 'POST body=JSON - 设置文件输入',
